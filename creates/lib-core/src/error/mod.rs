...
impl IntoResponse for AppError {
    fn into_response(self) -> axum::response::Response {
        let (status, body) = match self {
            AppError::NotFound => (StatusCode::NOT_FOUND, Res::error_with_message("not found")),
            AppError::Unauthorized => (
                StatusCode::UNAUTHORIZED,
                Res::error_with_message("UNAUTHORIZED"),
            ),
            AppError::InternalServerError => (
                StatusCode::INTERNAL_SERVER_ERROR,
                Res::error_with_message("Internal server error"),
            ),
            AppError::ServiceError(error_str) => {
                (StatusCode::OK, Res::error_with_message(error_str))
            }
            AppError::DbError(sql_error) => {
                error!("sql error:{:?}", sql_error);
                (StatusCode::OK, Res::error_with_message("查询失败"))
            }
            ...
            AppError::ParseError(parse_error) => {
                ...
                (StatusCode::OK, Res::error_with_message("参数错误"))
            }
            ...
            AppError::ParseIntError(parse_error) => {
                ...
                (StatusCode::OK, Res::error_with_message("系统内部错误"))
            }
